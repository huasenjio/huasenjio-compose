...
1. 云服务器要求至少 1 核 2G，带宽 2M+，否则首次加载过慢；
...
> ⚠️ 打开弹幕和查看评论区，演示系统为 CentOS 7.x，建议 7.4。
>
...
```bash
花森起始页：http://ip/portal/
后台管理系统：http://ip/admin/
```
...
进入项目根目录，打开`docker-compose.yml`，找到 nginx 的容器端口配置，代码段如下所示：
...
**修改左边宿主机的端口**，建议选 3000-10000 范围，然后执行 /huasen-compose/bin/update.sh 脚本更新配置。

（15）如何备份迁移网站？

执行停止脚本/huasen-compose/bin/stop.sh，通过压缩整个项目文件夹，拷贝迁移至新服务器，然后执行/huasen-compose/bin/recover.sh 脚本，自动安装依赖，启动网站。

（16）显示部署成功之后，我访问网站，老是打开 up 的主页？

遇到重定向到主页的情况，说明网站部署没问题，只是**访问的地址不对**，根据 `http://ip:nginx容器宿主机端口/portal/` 格式检查一下。例如：`http://127.0.0.1/portal/`，截止 2023 年 9 月 1 日，在后台管理已支持动态配置重定向地址，打开系统配置页面，设置重定向链接。

（17）我已经添加网链，为什么官网不显示链接呢？

官网不显示添加网链的原因主要有两种：17.2 网链没有绑定栏目，栏目没有绑定订阅源；17.2.权限不足，网链、栏目、订阅源任意一项权限不满足，就无法显示网链；详细视频教程：`https://www.bilibili.com/video/BV1ph411j7gP/`，建议花点时间了解其他介绍视频。
...
提交格式为`标识符+英文冒号+空格+模块+内容`，即：`feat: [首页]添加登录功能`；`fix: [后台管理]更新博客文章失败`；`refactor: [全局]重构安全加密传输方法`，请认真组织提交信息，便于后期管理及回溯！
...
📮 QQ 邮箱 ：[184820911@qq.com](184820911@qq.com)
...
| 奔儿啵儿霸           | --                                                                             | 🥤\*10   | 2024/11/10   | --                                                 |
| 略                   | --                                                                             | 🥤\*19   | 2024/11/10   | --                                                 |
| Dr. Alex Yu          | <img width="64px" src="https://s2.loli.net/2025/12/12/9uNWJYahj1Uifr6.png"  /> | 🥤\*3    | 2024/11/10   | --                                                 |
| 芜湖                 | <img width="64" src="https://s2.loli.net/2025/11/13/81NkxYzlgVrMtDa.png" />    | 🥤\*12   | 2025/11      | 大佬人非常好，耐心原创帮忙我解决问题，祝一路长虹！ |
| 赵辛没               | <img width="64" src="https://s2.loli.net/2025/11/13/HN4UVZTGKY8EJeu.png" />    | 🥤\*10   | 2025/11      | 希望花森越来越好                                   |

### 贡献榜

> 好事要留名，记得联系我，非常感谢您的无私奉献！

| 贡献人（自定义昵称） | 头像                                                                                         | 贡献内容                                 | 价值 | 时间    |
| -------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------- | ---- | ------- |
| Dr. Alex Yu          | <img width="64px" height="64px" src="https://s2.loli.net/2025/12/12/9uNWJYahj1Uifr6.png"  /> | 提供腾讯云服务器，用于测试快捷安装脚本。 | --   | 2025/11 |
