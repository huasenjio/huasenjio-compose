...
Form.prototype.onParseHeadersEnd = function(offset) {
  ...
  switch(self.partTransferEncoding){
    ...
    case 'base64': break;
    default:
      return createError(400, 'unknown transfer-encoding: ' + self.partTransferEncoding, {tag: "encodin", value: self.partTransferEncoding});
  }

  self.totalFieldCount += 1;
  if (self.totalFieldCount > self.maxFields) {
    return createError(413, 'maxFields ' + self.maxFields + ' exceeded', {tag: "maxFields", value: self.maxFields});
  }
  ...
  fs.open(publicFile.path, 'wx', function(err, fd) {
    ...
    internalFile.ws.on('error', function(err) {
      self.handleError(err.code === 'ETOOBIG'
        ? createError(413, err.message, {tag: "length", value: self.maxFilesSize})
        : err)
    });
...
function handleField(self, fieldStream) {
  ...
  fieldStream.on('readable', function() {
    ...
    if (self.totalFieldSize > self.maxFieldsSize) {
      self.handleError(createError(413, 'maxFieldsSize ' + self.maxFieldsSize + ' exceeded', {tag: "maxFieldsSize", value: self.maxFieldsSize}));
      return;
    }
