...
<template>
  ...
  >
    ...
    <slot></slot>
    <!-- 底部插槽 -->
    <span v-if="showFooter" slot="footer" class="hs-dialog__footer" :class="`is-${footerExtraAlign}`">
      <span class="hs-dialog__footer-extra">
        <slot name="footer-extra"></slot>
      </span>
      <span class="hs-dialog__footer-actions">
        <el-button size="small" v-if="buttons.cancel" @click="cancelDialog">
          {{ buttons.cancel }}
        </el-button>
...
::v-deep .el-dialog {
  ...
  .el-dialog__footer {
    ...
    .hs-dialog__footer-extra,
    .hs-dialog__footer-actions {
      display: flex;
      align-items: center;
      gap: 10px;
    }
    .hs-dialog__footer .el-button + .el-button {
      margin-left: 0;
    }
    .hs-dialog__footer-actions {
      flex-shrink: 0;
    }
    .hs-dialog__footer.is-right {
