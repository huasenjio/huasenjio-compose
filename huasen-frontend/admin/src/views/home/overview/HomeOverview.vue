...
<template>
  <div class="home-overview">
    <section v-for="(item, index) in Object.values(overviewMap)" :key="index" :style="{ backgroundColor: item.color }">
      <div class="left">
        <div class="label text">{{ item.label | emptyTip }}</div>
        <div class="value text">{{ overviewData[item.key] | emptyTip }}</div>
        <div class="rate text">{{ overviewData[item.rateKey] | emptyTip }} 同昨日相比</div>
      </div>
      <div class="right">
        <i :class="item.icon"></i>
      </div>
