 ...
 * @LastEditTime: 2022-11-04 00:38:02
 * @Description: 
-->
<template>
  <div v-rightMenu class="home-site">
    <section :id="`site-anchor-${encodeURI(index)}`" v-for="(item, index) in categorySites" :key="index">
...
export default {
  ...
  methods: {
    iconStyle(site) {
      let url = site.icon ? site.icon : require('@/assets/img/loading/1.gif');
      return {
        backgroundImage: `url(${url})`,
      };
    },
    imgUrl(site) {
      return site.icon ? site.icon : require('@/assets/img/loading/1.gif');
    },
  },
};
</script>
