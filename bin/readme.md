...
适用版本：centos 7.x、centos 8.x、ubuntu 22.04、debian 12、opencloudos 9.x
...
> 本仓库的升级脚本为最新版本，建议将它替换您服务器的`bin`目录下的`upgrade.sh`脚本之后，再执行升级，以确保您获得更好的升级体验。

拉取仓库代码，重新构建全部镜像，排除 mongodb 数据、redis 数据、jenkins 程序文件，更新全部文件，合并静态文件夹，升级网站。

### install.sh

网站部署脚本，请确保 **8081、3000、37017、7379、8181** 端口无占用服务，执行期间，如果 git、docker、docker-compose 不存在，就会安装，执行时发生报错或阻塞，大概率是因为网络问题，使用 `ctrl + c` 中断程序，然后**重新执行**脚本。
